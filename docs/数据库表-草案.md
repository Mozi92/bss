...
| `users 1 ─ 1 promotion_relations` | 一个用户对应一条推荐关系 |
| `users 1 ─ n reward_records` | 用户可有多条推广奖励记录 |
| `users 1 ─ 1 points_accounts` | 一个用户对应一个积分账户 |
| `points_accounts 1 ─ n points_transactions` | 积分账户有多条流水 |
| `users 1 ─ n withdraw_requests` | 用户可有多条兑换申请 |
...
### 3.6 promotion_relations — 推荐关系表

| 字段 | 类型 | 可空 | 默认值 | 说明 |
|------|------|------|--------|------|
| id | BIGSERIAL | 否 | 自增 | 主键 |
| user_id | BIGINT | 否 | — | 用户ID，外键→users.id，唯一 |
| direct_parent_id | BIGINT | 是 | NULL | 推荐人ID（谁推荐的），外键→users.id |
| parent_node_id | BIGINT | 是 | NULL | 上一层挂靠节点ID，外键→users.id |
| position | SMALLINT | 是 | NULL | 在推荐人名下的序号：1/2/3 |
...
| total_spent | INTEGER | 否 | 0 | 累计消耗积分（兑换+扣减） |
| created_at | TIMESTAMPTZ | 否 | now() | 创建时间 |
| updated_at | TIMESTAMPTZ | 否 | now() | 更新时间 |
...
| reward_record_id | BIGINT | 是 | NULL | 关联推广奖励记录ID，外键→reward_records.id |
| transaction_type | VARCHAR(30) | 否 | — | 类型：direct_reward-直推奖励, indirect_reward-间推奖励, unfreeze-解冻, freeze-冻结, withdraw-兑换扣减, refund_deduct-退款扣减, manual_adjust-手动调整, expire-过期 |
...
### 3.9 reward_records — 推广奖励记录表

| 字段 | 类型 | 可空 | 默认值 | 说明 |
|------|------|------|--------|------|
| id | BIGSERIAL | 否 | 自增 | 主键 |
| order_id | BIGINT | 否 | — | 订单ID，外键→orders.id |
| order_item_index | INTEGER | 否 | 0 | 订单内第几件（间推用，直推为0表示整单） |
| beneficiary_user_id | BIGINT | 否 | — | 获得奖励用户ID，外键→users.id |
| source_user_id | BIGINT | 否 | — | 下单用户ID，外键→users.id |
| reward_type | VARCHAR(20) | 否 | — | 类型：direct-直推奖励, indirect-间推奖励 |
| amount | INTEGER | 否 | — | 奖励积分金额 |
| level | INTEGER | 否 | 0 | 层级（0=直推奖励，1=上一层间推奖励，依次递增） |
...
### 3.10 withdraw_requests — 兑换申请表

| 字段 | 类型 | 可空 | 默认值 | 说明 |
|------|------|------|--------|------|
| id | BIGSERIAL | 否 | 自增 | 主键 |
| user_id | BIGINT | 否 | — | 用户ID，外键→users.id |
| amount | INTEGER | 否 | — | 兑换积分金额 |
...
| direct_reward_position_1 | 20 | 第1位推荐用户直推奖励比例（%） |
| direct_reward_position_2 | 20 | 第2位推荐用户直推奖励比例（%） |
| direct_reward_position_3 | 60 | 第3位推荐用户直推奖励比例（%） |
| indirect_reward_amount | 30 | 间推固定积分/件 |
| min_withdraw_points | 100 | 最低兑换积分 |
| max_withdraw_per_month | 2 | 每月最大兑换次数 |
...
| 7 | users | promotion_relations | 1:1 | promotion_relations.user_id | 用户推荐关系 |
| 8 | users | promotion_relations | 1:N | promotion_relations.direct_parent_id | 作为推荐人 |
...
| 13 | reward_records | points_transactions | 1:N | points_transactions.reward_record_id | 推广奖励关联流水 |
| 14 | orders | reward_records | 1:N | reward_records.order_id | 订单推广奖励记录 |
| 15 | users | reward_records | 1:N | reward_records.beneficiary_user_id | 获得推广奖励 |
| 16 | users | reward_records | 1:N | reward_records.source_user_id | 来源下单用户 |
| 17 | users | withdraw_requests | 1:N | withdraw_requests.user_id | 用户兑换申请 |
